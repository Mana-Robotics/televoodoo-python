"""UDP service for Televoodoo pose streaming.

This module provides a transport-agnostic UDP server with mDNS discovery.
It works for both WiFi and USB connections - the phone app discovers the
server via mDNS on whatever network interface it's connected to.
...
# Default UDP port for Televoodoo server
DEFAULT_PORT = 50000
...
_active_server: "Optional[UdpServer]" = None
...
class UdpServer:
    ...
    connections. It binds to all interfaces (0.0.0.0) and advertises
    via mDNS so the phone app can discover it.
    """
    ...
    def _start_mdns(self) -> None:
        ...
        try:
            ...
            # Service instance name - just the name, phone discovers via <name>._televoodoo._udp.local.
...
def run_server(
    ...
    - {"type": "disconnected", "reason": "timeout"|"bye"|"server_shutdown"}
    """
    server = UdpServer(
        name=name,
        code=code,
...
def get_server_url(port: int = DEFAULT_PORT) -> str:
    """Get the UDP endpoint info for the server.
...
def send_haptic(value: float, min_value: float = 0.0, max_value: float = 1.0) -> bool:
    ...
    # Get active UDP server (thread-safe)
    with _server_lock:
